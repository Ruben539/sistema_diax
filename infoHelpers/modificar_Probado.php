...
if (!empty($_POST)) {
	...
	if (empty( $_POST['nro_partida']) || empty($_POST['observacion']) || empty($_POST['falla_detectada']) ||empty($_POST['modelo'])) {

		$alert = '<p class = "msg_error">Debe llenar Todos los Campos</p>';

	}else{

		$id_verProbado    = $_POST['id'];
...
//exit; sirve para ejectuar la consulta en mysql
		$query = mysqli_query($conection,"SELECT * FROM verificacion_probado
			WHERE  id_verProbado != id_verProbado"
		);
	...
	if ($resultado > 0) {
		...
		$sql_update = mysqli_query($conection,"UPDATE verificacion_probado SET modelo = '$modelo',nro_partida = '$nro_partida',falla_detectada = '$falla_detectada'
        ,cantidad = '$cantidad',observacion = '$observacion', estatus = 1
			WHERE id_verProbado = $id_verProbado");

		if ($sql_update) {

			header('location: ../Informes/verificador_Probado.php');
...
$id_verProbado = $_REQUEST['id'];

$sql = mysqli_query($conection,"SELECT * FROM verificacion_probado WHERE id_verProbado = $id_verProbado AND estatus= 1");
...
<main class="app-content">
      <div class="row" style="justify-content: center;">
        <div class="col-md-5">
          <div class="tile">
            <h3 class="tile-title">Registro de Probado</h3>
            <div class="tile-body">
              <form action="" method="POST">
                  <input type="hidden" name="id" id="id" value="<?php echo $id_verProbado; ?>">
                ...
                <div class="form-group">
                  ...
                  <input class="form-control" type="number" name="nro_partida" id="nro_partida" placeholder="Ingrese el Nro de Partida"
                  value="<?php echo $nro_partida; ?>" required>
                </div>
              
                <div class="form-group">
                  <label class="control-label">Falla Detectada</label>
                   <select class="form-control" id="falla_detectada" name="falla_detectada" required>
                        <option value="<?php echo $falla_detectada; ?>"><?php echo $falla_detectada; ?></option>
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" rows="4" name="observacion" id="observacion"  placeholder="Ingrese la observacion" style="height: 45px;"><?php echo $observacion; ?></textarea>
                </div>
