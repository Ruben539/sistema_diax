...
if (!empty($_POST)) {
	...
	if (empty( $_POST['nro_partida']) || empty($_POST['sector']) || empty($_POST['falla_detectada']) ||empty($_POST['modelo'])) {

		$alert = '<p class = "msg_error">Debe llenar Todos los Campos</p>';

	}else{

		$id_verProduccion = $_POST['id'];
...
//exit; sirve para ejectuar la consulta en mysql
		$query = mysqli_query($conection,"SELECT * FROM verificacion_produccion
			WHERE  id_verProduccion != id_verProduccion"
		);
	...
	if ($resultado > 0) {
		...
		$sql_update = mysqli_query($conection,"UPDATE verificacion_produccion SET modelo = '$modelo',nro_partida = '$nro_partida', sector = '$sector',pieza = '$pieza',falla_detectada = '$falla_detectada',
        solucion = '$solucion',cantidad = '$cantidad', estatus = 1
			WHERE id_verProduccion = $id_verProduccion");

		if ($sql_update) {

			header('location: ../Informes/info_produccion.php');
...
$id_verProduccion = $_REQUEST['id'];

$sql = mysqli_query($conection,"SELECT * FROM verificacion_produccion WHERE id_verProduccion = $id_verProduccion AND estatus= 1");
...
<main class="app-content">
      <div class="row" style="justify-content: center;">
        <div class="col-md-5">
          <div class="tile">
            <h3 class="tile-title">Registro de Produccion</h3>
            <div class="tile-body">
              <form action="" method="POST">
                  <input type="hidden" name="id" id="id" value="<?php echo $id_verProduccion; ?>">
                ...
                <div class="form-group">
                  <label class="control-label">Sector Responsable</label>
                  <input class="form-control" type="text" name="sector" id="sector" placeholder="Ingrese el Responsable"
                  value="<?php echo $sector; ?>" required>
                </div>
                <div class="form-group">
                  <label class="control-label">Pieza</label>
                   <select class="form-control" id="pieza" name="pieza" required>
                        <option value="<?php echo $pieza; ?>"><?php echo $pieza; ?></option>
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" rows="4" name="falla_detectada" id="falla_detectada"  placeholder="Ingrese la Falla" style="height: 45px;" ><?php echo $falla_detectada; ?></textarea>
                </div>
                <div class="form-group">
                  <label class="control-label">Solucion</label>
                  <textarea class="form-control" rows="4" name="solucion" id="solucion"  placeholder="Ingrese la Solucion" style="height: 45px;"><?php echo $falla_detectada; ?></textarea>
                </div>
              ...
              <button class="btn btn-primary" type="submit"><i class="fa fa-fw fa-lg fa-check-circle"></i>Registrar</button>&nbsp;&nbsp;&nbsp;<a class="btn btn-secondary" href="../Informes/info_produccion.php"><i class="fa fa-fw fa-lg fa-times-circle"></i>Cancelar</a>
