                  ...
                  <tbody>
                      <?php 
                        $hoy = date('m-Y');
                         ...
                         $sql = mysqli_query($conection, "SELECT h.id,h.Cedula,c.nombre,c.apellido,h.estudio,h.atendedor,h.cancelado,h.fecha FROM historial h INNER JOIN clientes c on c.cedula = h.cedula
                         WHERE h.cancelado != ' ' AND fecha like '%".$hoy."%' ORDER BY ID ASC ");
